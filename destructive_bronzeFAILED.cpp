/*
League: Bronze (created before silver league was opened)
Spells used: All spells
Tactic: Destructive (v1 less agression)
Two heroes use the Barrier tactic: guard around the base and using wind spell to push monsters away from base
One hero destroys opponent's base by:
- wind spell to opponent base if monster is outside of the base
- shield spell a monster that has locked on to opponent base
- control spell to opponent hero to move away from monsters (to be explored in detail)
*/
...
int dx[] = {2000, 4000, 17630 - 4500};
int dy[] = {4000, 2000, 9000 - 4500};
        ...
        for(int i = heroes_per_player - 1; i < heroes_per_player; i ++){
            ...
            if(what_to_do && our_mana >= 20){
                ...
                if((herox - x) * 1ll * (herox - x) + (heroy - y) * 1ll * (heroy - y) >= 2000ll * 2000ll)
                    ...
                    ll get_opponent_distance = (entity_list[nearest_entity].x - (17630 - base_x)) * 1ll * (entity_list[nearest_entity].x - (17630 - base_x)) + (entity_list[nearest_entity].y - (9000 - base_y)) * (entity_list[nearest_entity].y - (9000 - base_y));
                    if(get_opponent_distance <= 5500ll * 5500ll){
