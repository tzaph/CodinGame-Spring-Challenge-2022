/*
League: Bronze -> Silver [best rank 18/3226 25 hours after start]
Spells used: All spells
Tactic: Sky Arrow V2
Simply one defender, one farmer, and one attacker. 
Optimizsed Farmer and Attacker:
- Attacker farms early game
- Attacker controls spiders to different directions
- Farmer moves back to defend midgame
- Defender farms with farmer early game
*/
...
int tx[] = {1000, 3000, 4000};
int ty[] = {4000, 3000, 1000};
...
int main(){
    ...
    for(;;){
        ...
        for(int i = heroes_per_player - 2; i < heroes_per_player - 1; i ++){
            if(turn_left <= 150){
                int x = (base_x + dy[i - 1] <= 17630 ? base_x + dy[i - 1] : base_x - dy[i - 1]), y = (base_y + dx[i - 1] <= 9000 ? base_y + dx[i - 1] : base_y - dx[i - 1]), threatfor = 0, nearbase = 0;
                ll distance = inf; int target_entity = -1;
                ll base_distance = inf;
                int herox = li[i].second.first, heroy = li[i].second.second;
                int idhero = li[i].first;
                ...
                if(target_entity != -1){
                        ...
                        bool can_wind = 0;
                        for(pair<pair<ll, ll>, int> i : lis) if(i.first.first <= 1100ll * 1100ll && i.second == 0) can_wind = 1;
                        if(our_mana >= 10 && can_wind && !wind_done_defending){
                                use_wind = 1;
                        }
                }
                if(use_wind){
            ...
            if(turn_left <= 140){
                ...
                for(int j = 0; j < entity_list.size(); j ++) if(entity_list[j].type == 0){
                    ...
                    if(get_distance <= 2100ll * 2100ll && our_mana >= 10){
                        if(entity_list[j].threat_for == 2 && entity_list[j].shield_life == 0 && get_base_distance <= 5500ll * 5500ll){
                            what_to_do = 1; target_entity = j;
                        }else if(our_mana >= 60 && entity_list[j].threat_for != 2 && what_to_do != 1){
