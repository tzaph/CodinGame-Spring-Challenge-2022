/*
League: Gold (rank 37th in Gold, rank 96th global)
Spells used: All spells
Tactic: True StartEnd S1
	This is the start and this is the end

- Optimising Skyarrow v4
- Placing attackers at the edge and stay in place
	- A monster is controlled to 1040 units to the back of the two heroes
	- When reached, use one wind spell, then two wind spells
- A logical attack would attack from the long side (longer = more monster rate)
	- hence a logical defense would push the monsters away from the long side to prevent backfiring
	- In other words, wind to the short side
- Health can be a metric of danger and in other words become a guide of mana cap for wind in defense:
	- Less opponent base health: leading, lower mana cap to strengthen defense
	- Less our base health: losing, lower mana cap to strengthen defense
- Tidying up code
*/
...
void defense0_pattern(){
	...
	if(urgent_wind || (use_wind && phase2 && mana >= health * 15)){
		...
		if(control_target != -1 && phase2 && mana >= health * 15)
